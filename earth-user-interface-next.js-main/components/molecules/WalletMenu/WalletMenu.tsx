...
import CopyImage from '../../../public/link-copy.png'
      ...
      <div className={styles.walletMenu}>
        ...
        {hasPolkExtension && actualAccount && hasMultipleAccounts === false ? (
          ...
          <div className={styles.connectWallet}>
            ...
            <div className={`${styles.copyAddress} ${copyAddress ? styles.copiedAddress : ''}`} onClick={() => {navigator.clipboard.writeText(actualAccount.address); setCopyAddress(true);}}><Image src={CopyImage} alt={"Copy image button"}></Image></div>
          </div>
        ...
        {actualAccount ? (
          <button onClick={ () => {disconnectWallet(); setCopyAddress(false); }} className={styles.disconnectWallet}>Disconnect Wallet</button>
          ) : (
            <div></div>
            )}
      </div>
