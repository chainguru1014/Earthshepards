...
use shear_runtime::{constants::currency::*, BalancesCall};
...
fn extrinsic_set_time(now: u64) -> OpaqueExtrinsic {
	shear_runtime::UncheckedExtrinsic {
		signature: None,
		function: shear_runtime::Call::Timestamp(pallet_timestamp::Call::set { now }),
	}
	.into()
}
...
fn prepare_benchmark(client: &FullClient) -> (usize, Vec<OpaqueExtrinsic>) {
	...
	for nonce in 0.. {
		let extrinsic: OpaqueExtrinsic = create_extrinsic(
			client,
			src.clone(),
			BalancesCall::transfer { dest: dst.clone(), value: 1 * SHEAR },
			Some(nonce),
		)
		.into();
