...
pub fn config_endowed(code: Option<&[u8]>, extra_endowed: Vec<AccountId>) -> GenesisConfig {
	let mut endowed = vec![
		(alice(), 111 * SHEAR),
		(bob(), 100 * SHEAR),
		(charlie(), 100_000_000 * SHEAR),
		(dave(), 111 * SHEAR),
		(eve(), 101 * SHEAR),
		(ferdie(), 100 * SHEAR),
	];

	endowed.extend(extra_endowed.into_iter().map(|endowed| (endowed, 100 * SHEAR)));
	...
	GenesisConfig {
		...
		staking: StakingConfig {
			stakers: vec![
				(dave(), alice(), 111 * SHEAR, StakerStatus::Validator),
				(eve(), bob(), 100 * SHEAR, StakerStatus::Validator),
				(ferdie(), charlie(), 100 * SHEAR, StakerStatus::Validator),
			],
			validator_count: 3,
